...
var TemplateRenderer = function(site) {
  ...
  // console.log("Loading layouts from " + layoutDir);
  
  if (!fs.existsSync(layoutDir) ||
      !fs.statSync(layoutDir).isDirectory())
  {
    console.log("Layout path at " + layoutDir + " is not a directory");
    return;
  }
...
TemplateRenderer.prototype = {  
  openAndSplit: function(loc) {
    ...
    if (parts[0] === "---\n")
      ...
      try {
        ...
        console.log("Failed to parse yaml for " + loc + " with error " + ex);
        parts[0] = {};
      }
    }
    return parts;
  ...
  renderTemplate: function(meta, layout, cb) {
    ...
    if (templateLayout) {
      // console.log("template has parent template " + templateLayout);
...
File.prototype = {
  ...
  renderToFile: function() {
    ...
    that.renderer.render(that.loc, function(err, data) {
      ...
      // console.log("Rendering file at local path " + localPath);
      var outPath = path.join(that.site.publicDir, that.renderer.toOutPath(localPath));
      // console.log("Rendering file to global path " + outPath);
...
Site.prototype = {
  init: function() {
    ...
    if (!fs.existsSync(publicDir)) {
      console.log("ERROR: public directory '" + publicDir + "' does not exist");
      return false;
    }
    
    var publicDirStat = fs.statSync(publicDir);
    if (!publicDirStat || !publicDirStat.isDirectory()) {
      console.log("ERROR: public directory '" + publicDir + "' is not a directory");
      return false;
    }
    ...
    var processDir = function(dir) {
      ...
      subFiles.forEach(function(file) {
        ...
        if (fs.statSync(fullPath).isDirectory())
          ...
          if (!renderer) {
            console.log("Failed to find renderer for " + fullPath);
            return;
          }
  ...
  findRenderer: function(loc) {
    for (name in this.renderers) {
      ...
      if (renderer.canRender(loc)) {
        // console.log("Selected renderer " + name + " for file " + loc);
        return renderer;
      }
    }
    return false;
...
exports.live = function(config) {
  ...
  console.log("Serving live site on port " + config.port + " from src " + site.dir);
...
exports.serve = function(config) {
  ...
  console.log("Serving site on port " + config.port + " from dir " + site.publicDir);
